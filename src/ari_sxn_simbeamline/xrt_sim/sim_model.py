...
# Define a test object to use in place of the caproto IOC for testing (remove
# once ari_sim.py is fully functional)
mirror1 = TestM1({'Ry_coarse': -2, 'Ry_fine': 0, 'Rz': 0,
                  'x': 0, 'y': 0})

# Define optics coating material instances (remove once ari_sim.py is fully
# functional).
...
class BeamlineModel:
    ...
    def activate(self):
        ...
        for item in self.definition_dict.keys():
            getattr(self, item).activate(updated=True)  # remove updated=True when custom devices have been reparameterized
